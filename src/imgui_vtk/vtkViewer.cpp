...
// This needs to be used along with a Platform Binding (e.g. GLFW, SDL, Win32, custom..) and a
// renderer binding (OpenGL)
...
void VtkViewer::isCurrentCallbackFn(
    vtkObject* caller,
...
{
    if (((viewportWidth == newSize.x && viewportHeight == newSize.y) || viewportWidth <= 0 ||
         viewportHeight <= 0) &&
        !firstRender)
    {
        return;
